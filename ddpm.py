...
class NoiseScheduler():
    def __init__(self,
        ...
        if beta_schedule == "linear":
            ...
            s = 0.008
            f_t = torch.cos(((t / num_timesteps) + s) / (1.0 + s) * torch.pi * 0.5) ** 2
...
if __name__ == "__main__":
    ...
    val_dataloader = DataLoader(
        val_dataset, batch_size=config.train_batch_size, shuffle=False, drop_last=False)
    ...
    np.save(f"{outdir}/val_loss.npy", np.array(val_losses))
